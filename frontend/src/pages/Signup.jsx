...
const Signup = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3009/user/signup", {
        method: "POST",
        headers: {
  ...
  return (
    <div className='flex h-screen items-center justify-center'>
      ...
      <h1 className='text-green-600 font-bold text-2xl items-center'>Create your Account</h1>
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-3'>
         ...
         <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-lg border border-gray-400'>
            ...
            <input type="text" placeholder='Username' {...register("name", { required: true })}  className='grow outline-none' />
          </div>
          {errors.name && <span className='font-semibold text-sm text-red-600'>This field is required</span>}
          {/* email */}
          <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-lg border border-gray-400'>
          ...
          <input type={showPassword ? 'text' : 'password'} {...register("confirmPassword", { required: true, validate:validatePassword })}  placeholder='Confirm-Password' className='grow outline-none' />
          </div>
           {errors.confirmPassword && <span className='font-semibold text-sm text-red-600'>{errors.confirmPassword.message}</span>}
          <div className='flex justify-center'>
            <button type='submit' className='text-white bg-green-600 w-full rounded-lg py-2 cursor-pointer'>signup</button>
          </div>
          </form>
          <p className='mt-2 text-center'>Already a user ?? <span className='text-blue-500 underline cursor-pointer ml-1' onClick={(e)=>{e.preventDefault(),navigate('/login')}}>login</span></p>
          </div>
    </div>
  )
}
